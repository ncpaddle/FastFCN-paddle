...
class EncHead(nn.Layer):
    def __init__(self, in_channels, out_channels, se_loss=True, jpu=True, lateral=False,
        ...
        self.conv5 = nn.Sequential(nn.Conv2D(in_channels[-1], 512, 1, bias_attr=False),
                                   norm_layer(512),
                                   nn.ReLU()) if jpu else \
                     nn.Sequential(nn.Conv2D(in_channels[-1], 512, 3, padding=1, bias_attr=False),
                                   norm_layer(512),
                                   nn.ReLU())
        if lateral:
            self.connect = nn.LayerList([
                nn.Sequential(
                    nn.Conv2D(in_channels[0], 512, kernel_size=1, bias_attr=False),
                    norm_layer(512),
                    nn.ReLU()),
                nn.Sequential(
                    nn.Conv2D(in_channels[1], 512, kernel_size=1, bias_attr=False),
                    norm_layer(512),
                    nn.ReLU()),
            ])
        ...
        self.conv6 = nn.Sequential(nn.Dropout(p=0),
...
def get_encnet_resnet50_ade(pretrained=False, root='~/.encoding/models', **kwargs):
    ...
    >>> print(model)
    """
    return get_encnet('ade20k', 'resnet50', pretrained=False, root=root, aux=True,
                      base_size=520, crop_size=480, **kwargs)
